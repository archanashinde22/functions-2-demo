...
// crreating the high order function that wwill tak e the above function as callback . Its gonna check  and convert if arguments recieved are not numbers and then invoke the callback function
...
const calculator = (num1, num2, callback) => {
  if (+num1 && +num2) {
    ...
    console.log(`You need to send a numbers`);
  }
};

const result = calculator(`18`, 2, divide);
...
const applyPercentageDiscount = (product, discount) => {
  product.displayPrice = product.basePrice * (1 - discount / 100);
};
// flat rate = 5$ or $10
...
//applyDiscount(catProducts,applyPercentageDiscount,25);
...
// create a function that lets us applt discount based off of our inventory

const applyDiscountByInvetory = (arr, callback, discount, amount) => {
  arr.forEach((product) => {
...
//  applyDiscountByInvetory(dogProducts, applyFlatRateDiscount, 2, 80 )
...
//const makePumperNickleSandwich = makeSandwich(`pumperNickel`)

//console.log(makePumperNickleSandwich(['cream-cheese','peppers','capicola','seasoning']))
...
const productOfArray = (numsToReduce) =>
  numsToReduce.reduce((acc, curr) => acc + curr);
